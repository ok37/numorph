...
image_dim = size(ref_img);
if direction == 1
    %Horizontal image overlap regions
    overlap_min = {[1,image_dim(1)],[1,round(image_dim(2)*overlap)]};
    overlap_max = {[1,image_dim(1)],[image_dim(2)-overlap_min{2}(2)+1,image_dim(2)]};
else
    %Vertical image overlap regions
    overlap_min = {[1,round(image_dim(1)*overlap)],[1,image_dim(2)]};
    overlap_max = {[image_dim(1)-overlap_min{1}(2)+1,image_dim(1)],[1,image_dim(2)]};
end
