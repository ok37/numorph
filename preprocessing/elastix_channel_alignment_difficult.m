function elastix_channel_alignment_difficult(x,y,path_table,config)
...
max_chunk_size = 250; %Important parameters. Decreasing might improve precision but takes longer. Higher values usually gives a more reproducible results
chunk_pad = 50; %Important parameter. If misalignment is very large, you will have to increase to prevent zero areas from occuring in the output chunk edges
...
enhance_blobs = "ftrue"; %Enhance blob structures for more robust alignment
...
if isequal(using_loaded_parameters,'true')
    ...
    if max_chunk_size > size(I{1},3)
        chunk_start = 1;
        chunk_end = size(I{1},3);
        
        chunk_start_adj = 1;
        chunk_end_adj = size(I{1},3);
        
    elseif chunk_size < max_chunk_size
...
if isequal(using_loaded_parameters,'false')
    ...
    for i = 2:length(markers)-1 
        init_tform{i} = elastix(I{i+1}(:,:,1:300),I{1}(:,:,1:400),outputDir{i},...
            {transform_path_init},'s',s,'threads',[]);
    end
    ...
    for i = 1:n_chunks
        ...
        for j = 2:length(markers)-1
            ...
            fprintf('%s\t Aligning to slices %d to %d \n',datetime('now'),chunk_start2,chunk_end2)
            ...
            %imshowpair(reg_img(:,:,150),I_ref(:,:,150))
            
            %saveastiff(reg_img,'reg_img.tif')
            %imshowpair(I{1}(:,:,339-30),reg_img(:,:,end-30))
            ...
            I_raw = apply_transformations(I_raw,out(i,:),num_images,ncols,nrows,markers);
            
            % Save some sample images
            save_range = 100:100:num_images;
            s_img = ismember(save_range,chunk_start2:chunk_end2);
            for k = save_range(s_img)
                I_merge = cat(3,I_raw{1}(:,:,k),I_raw{2}(:,:,k),I_raw{3}(:,:,k));

                img_name = sprintf('merge_%s_0%d_0%d_aligned.tif',num2str(k,'%04.f'),y,x);
...
n_chunks = size(out,1);
chunk_start_adj = cellfun(@(s) s.chunk_start_adj,out(:,end));
chunk_end_adj = cellfun(@(s) s.chunk_end_adj,out(:,end));

chunk_start = cellfun(@(s) s.chunk_start,out(:,end));
chunk_end = cellfun(@(s) s.chunk_end,out(:,end));
...
for i = 1:n_chunks
    ...
    for j = 2:length(markers)-1
        ...
        nslices = out2.TransformParameters{end}.Size(end) * out2.TransformParameters{end}.Spacing(end);
        size1 = [ncols nrows nslices];
