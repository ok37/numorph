...
atlas_file = ["ara_nissl_25.nii",...
    "average_template_25.nii"];                 % Name of the atlas file to register to. Can provide full path or place .nii file in /data/atlas
...
%% Nuclei Detection
count_method = "3dunet";            % 3dunet, hessian. Use 3D-Unet (runs on GPU) or hessian blob detector (runs on CPU) 
measure_local_background = "load";  % true, false; measure local image background
min_intensity = [];                 % Minimum intensity for cell nuclei
chunk_size = [112,112,32];          % Do not edit if using 3D-Unet. Chunk size of unet model
chunk_overlap = [16,16,8];          % Do not edit if using 3D-Unet. Overlap between chunks
...
resample_chunks = "false";          % Resample images to match model resolution. This processing step takes significantly longer
