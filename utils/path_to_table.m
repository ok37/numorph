% This function takes a structure of image directories, identifies tile
% positions and marker names by regular expressions, and outputs a final
% table organized with relevant position and channel information for futher
% processing. The file identifiers used here are specific to those used by the 
% ImSpector software used by LaVision Biotech

function [path_table_main] = path_to_table(path,location,markers,channel_num,sample_name)
...
switch location
    ...
    case 'resampled'
        ...
        if ~isequal(length(path_sub), length(markers))
            msg = sprintf('%s\t Number of .nii files does not match number of specified markers \n',datetime('now'));
            error(msg)
        end
